...
async def check_fraud_with_resilience(user_id: int, amount: float):
    """
    Handles Distributed Tracing, Retries, and Explicit Fail-Open Logic
    """
    ...
    async with httpx.AsyncClient(timeout=3.0) as client:
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logger.info(f"[{tx_id}] Fraud Raw Response: {fraud_data}")
                
                risk_score = fraud_data.get("risk_score")
                decision = fraud_data.get("decision")

                # UPGRADE: Explicit Logic for Contract Mismatch
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    # UPGRADE: Explicit Logic for Service Unreachable
...
async def process_transfer(db: Session, sender: models.User, receiver_email: str, amount: float):
    ...
    # 2. Refactored Fraud Check (Upgraded to Explicit Dictionary Handling)
    fraud_result = await check_fraud_with_resilience(sender.id, amount)
    
    if fraud_result.get("is_fraud"):
        logger.warning(f"[{tx_id}] Sentinel Security: Transaction BLOCKED. Reason: {fraud_result.get('reason')}")
        raise HTTPException(status_code=403, detail="Sentinel Security: Transaction blocked.")
    
    if "fail_open" in fraud_result.get("reason", ""):
        logger.info(f"[{tx_id}] Security Note: Transaction allowed via Fail-Open strategy ({fraud_result.get('reason')})")
    ...
    try:
        ...
        # --- TASK 3: ASYNCHRONOUS NOTIFICATION ---
        # We fire this off to Redis. The user doesn't wait for the 5-second sleep!
